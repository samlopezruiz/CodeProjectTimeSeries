...
def dcnn_multi_step_uv_fit(train, cfg, plot_hist=False, verbose=0):
    # unpack config
    ...
    # define model
    model = dcnn_build(cfg, n_features=1)
...
def dcnn_multi_step_uv_predict(model, history, cfg, steps=1):
    # unpack config
...
# forecast with a pre-fit model
def dcnn_multi_step_uv_predict_walk(model, history, cfg, steps=1):
    # unpack config
