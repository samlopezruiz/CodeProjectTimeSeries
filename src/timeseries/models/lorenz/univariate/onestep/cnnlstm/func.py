...
def cnnlstm_one_step_uv_fit(train, cfg):
    ...
    # prepare data
    X, y = step_feature_one_step_xy_from_uv(train, n_input, 1, n_seq)
    # define model
    model = Sequential()
...
def cnnlstm_one_step_uv_predict(model, history, cfg, steps=1):
    # unpack architectures
