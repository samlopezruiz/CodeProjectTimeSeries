...
from algorithms.wavenet.func import dcnn_build, dcnn_build2
...
def dcnn_get_multi_step_mv_funcs():
    return [dcnn_multi_step_mv_predict, dcnn_multi_step_mv_fit, dcnn_build]

def dcnn_get_multi_step_mv_funcs2():
    return [dcnn_multi_step_mv_predict, dcnn_multi_step_mv_fit2, dcnn_build2]


def dcnn_multi_step_mv_fit2(train, cfg, plot_hist=False, verbose=0):
    # unpack architectures
    n_steps_in, n_steps_out = cfg['n_steps_in'], cfg['n_steps_out']
    n_epochs, n_batch = cfg['n_epochs'], cfg['n_batch']
    # prepare data
    ...
    n_features = X.shape[2]
    # define model
    model = dcnn_build2(cfg, n_features)
    model.compile(loss='mse', optimizer='adam')
    if verbose > 0:
        print('No. of params: {}'.format(model.count_params()))
    # fit
