...
def wavenet_mv_configs(steps=1):
    keys = {6: 0, 3: 1, 1: 2}
    good_configs = []
    ...
    functions = wavenet_get_multi_step_mv_funcs()
    ...
    # %% WAVENET: 0.9302 minmax  (+/- 0.004) STEPS=6
    model_name = "WAVENET"
    input_cfg = {"variate": "multi", "granularity": 5, "noise": True, 'preprocess': True,
                 'trend': True, 'detrend': 'ln_return'}
    model_cfg = {"n_steps_in": 15, "n_steps_out": 6, 'n_layers': 3, "n_filters": 64,
                 "n_kernel": 3, "n_epochs": 20, "n_batch": 100, 'hidden_channels': 7}
    functions = wavenet_get_multi_step_mv_funcs()
    good_configs.append((model_name, input_cfg, model_cfg, functions))
