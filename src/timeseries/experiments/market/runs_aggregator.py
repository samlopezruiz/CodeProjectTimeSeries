...
if __name__ == '__main__':
    ...
    runs_path = 'outputs/saved_models/snp/60t_macd/logs/fit'
    output = 'outputs/runs/snp/60t_macd'
    subpaths = ['train', 'validation']

    # aggregate(runs_path, output, subpaths)
#%%
    subpath = 'train'
    scalar_accumulators = []
    for dname in os.listdir(runs_path):
        run_path = os.path.normpath(os.path.join(runs_path, dname, subpath))
        scalar_accumulators.append(EventAccumulator(run_path).Reload())

    scalar_accumulator = scalar_accumulators[0]

    scalar_accumulator.scalars