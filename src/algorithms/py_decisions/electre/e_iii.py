        ...
        for i in range(0, global_concordance.shape[0]):
            for j in range(0, global_concordance.shape[1]):
                if (dataset[j, k] - dataset[i, k] >= P[k]):
                    ...
                    global_concordance[i, j] = global_concordance[i, j] + W[k] * (
                                (P[k] - dataset[j, k] + dataset[i, k]) / (P[k] - Q[k]))
    if (np.sum(W) != 0):
...
def destilation_descending(credibility):
    ...
    while len(alts) > 0:
        ...
        if (np.where(qual == np.amax(qual))[0].shape[0] > 1):
            ...
            while (np.where(qual_tie == np.amax(qual_tie))[0].shape[0] > 1 and
                   np.where(qual_tie == np.amax(qual_tie))[0].shape[0] < np.where(qual == np.amax(qual))[0].shape[0]):
...
def destilation_ascending(credibility):
    ...
    while len(alts) > 0:
        ...
        if (np.where(qual == np.amin(qual))[0].shape[0] > 1):
            ...
            credibility_tie = credibility[index[:, None], index]
            qual_tie = qualification(credibility_tie)
            while (np.where(qual_tie == np.amin(qual_tie))[0].shape[0] > 1 and
                   np.where(qual_tie == np.amin(qual_tie))[0].shape[0] < np.where(qual == np.amin(qual))[0].shape[0]):
                qual = qualification(credibility_tie)
...
def pre_order_matrix(rank_D, rank_A, number_of_alternatives=7):
    ...
    for i in range(0, number_of_alternatives):
        for j in range(0, number_of_alternatives):
            if (i < j):
                if ((alts_D[i] < alts_D[j] and alts_A[i] < alts_A[j]) or (
                        alts_D[i] == alts_D[j] and alts_A[i] < alts_A[j]) or (
                        alts_D[i] < alts_D[j] and alts_A[i] == alts_A[j])):
                    po_string[i, j] = 'P+'
                    po_string[j, i] = 'P-'
                if ((alts_D[i] > alts_D[j] and alts_A[i] > alts_A[j]) or (
                        alts_D[i] == alts_D[j] and alts_A[i] > alts_A[j]) or (
...
def po_ranking(po_string):
    ...
    for i in range(0, rank_xy.shape[0]):
        plt.text(rank_xy[i, 0], rank_xy[i, 1], alts_rank[i], size=12, ha='center', va='center',
    ...
    for i in range(0, rank.shape[0]):
        for j in range(0, rank.shape[1]):
            ...
            if (rank[i, j] == 1):
                plt.arrow(rank_xy[k1, 0], rank_xy[k1, 1], rank_xy[k2, 0] - rank_xy[k1, 0],
                          rank_xy[k2, 1] - rank_xy[k1, 1], head_width=0.01, head_length=0.2, overhang=0.0,
                          color='black', linewidth=0.9, length_includes_head=True)
    axes = plt.gca()
