    ...
    for i in range(0, dominance_s.shape[0]):
        for j in range(0, dominance_s.shape[1]):
            if (concordance[i, j] >= concordance[j, i] and i != j):
                if (((concordance[i, j] >= c_plus) and (discordance[i, j] <= d_plus)) or (
                        (concordance[i, j] >= c_zero) and (discordance[i, j] <= d_minus))):
                    dominance_s[i, j] = 1
                if ((concordance[i, j] >= c_zero and discordance[i, j] <= d_plus) or (
                        concordance[i, j] >= c_minus and discordance[i, j] <= d_minus)):
                    dominance_w[i, j] = 1
...
def electre_ii(dataset, W, c_minus=0.65, c_zero=0.75, c_plus=0.85, d_minus=0.25, d_plus=0.50, graph=False):
    ...
    dominance_s, dominance_w = dominance_matrix(concordance, discordance, c_minus=c_minus, c_zero=c_zero, c_plus=c_plus,
