...
from algorithms.gpregress.classes import Individual, Tree, generate
from algorithms.gpregress.func import stats, to_df, print_crossover
from algorithms.gpregress.standalone import print_tree
from algorithms.gpregress.tree import get_subtrees, substitute_subtrees, rand_node, find_subtree, find_subtree_by_id


def initialize_pop(pop_size, primitives, n_terminals, depth=10):
    id_count = 0
    pop = []
    for _ in range(pop_size):
        ind = Individual(n_terminals, primitives, depth, n=1, id=id_count)
...
def train_gpregress(n_gen, n_terminals, primitives, depth, X, y, n_pop=100, selec=selection_roullete,
        ...
        for i in range(elitism_size, n_pop-1, 2):
            if np.random.rand() < cxpb:
                ...
                if np.random.rand() < mxpb:
                    selected[i], id_count = mutate(selected[i], primitives, n_terminals, id_count, depth)
                if np.random.rand() < mxpb:
                    selected[i+1], id_count = mutate(selected[i+1], primitives, n_terminals, id_count, depth)
...
def mutate(individual, primitives, n_terminals, id_count, depth):
    ...
    if node_id is not None:
        ...
        new_ind = Individual(n_terminals, primitives, allowed_depth, id=id_count)
