   ...
   This is the Hidden Markov Model Class
    ...
    def viterbi_inference(self, obs_seq):
        ...
        with tf.name_scope('Starting_log-priors'):
            ...
            pathScores = tf.tensor_scatter_nd_update(pathScores, 0, tf.math.log(self.T0) + tf.squeeze(obs_prob_list[0]))
        ...
        with tf.name_scope('Belief_Propagation'):
            for step, obs_prob in enumerate(obs_prob_list[1:]):
                with tf.name_scope('Belief_Propagation_step_%s' % step):
                    ...
                    pathStates = tf.tensor_scatter_nd_update(pathStates, step + 1, tf.argmax(belief, 0))
                    pathScores = tf.tensor_scatter_nd_update(pathScores, step + 1,
                                                   tf.reduce_max(belief, 0) + tf.squeeze(obs_prob))

            with tf.name_scope('Max_Likelyhood_update'):
                # infer most likely last state
                states_seq = tf.tensor_scatter_nd_update(states_seq, self.N - 1, tf.argmax(pathScores[self.N - 1, :], 0))
        ...
        with tf.name_scope('Backtrack'):
            for step in range(self.N - 1, 0, -1):
                with tf.name_scope('Back_track_step_%s' % step):
                    ...
                    states_seq = tf.tensor_scatter_nd_update(states_seq, step - 1, state_prob[0])
    ...
    def _forward(self, obs_prob_list):
        ...
        with tf.name_scope('forward_first_step'):
            ...
            self.scale = tf.tensor_scatter_nd_update(self.scale, 0, 1.0 / tf.reduce_sum(init_prob))

            # scaled belief at t=0
            self.forward = tf.tensor_scatter_nd_update(self.forward, 0, self.scale[0] * init_prob)
        ...
        for step, obs_prob in enumerate(obs_prob_list[1:]):
            with tf.name_scope('time_step-%s' % step):
                ...
                # scaling factor
                self.scale = tf.tensor_scatter_nd_update(self.scale, step + 1, 1.0 / tf.reduce_sum(forward_prob))
                # Update forward matrix
                self.forward = tf.tensor_scatter_nd_update(self.forward, step + 1, self.scale[step + 1] * forward_prob)

    def _backward(self, obs_prob_list):
        with tf.name_scope('backward_last_step'):
            # initialize with state ending priors
            self.backward = tf.tensor_scatter_nd_update(self.backward, 0,
        ...
        for step, obs_prob in enumerate(obs_prob_list[:-1]):
            with tf.name_scope('time_step-%s' % step):
                ...
                self.backward = tf.tensor_scatter_nd_update(self.backward, step + 1,
                ...
                for i in range(self.S):
                    with tf.name_scope('State-%s' % i):
                        numer = self.forward[t, i] * self.T[i, :] * tf.gather(self.E, x[t + 1]) * self.backward[t + 1,
                                                                                                  :]
                        trans_re_estimate = tf.tensor_scatter_nd_update(trans_re_estimate, i, numer / denom)

                self.M = tf.tensor_scatter_nd_update(self.M, t, trans_re_estimate)
